...
所有延迟单位均为毫秒。服务器横向对比使用 NVIDIA H800 80GB，Search 和 Add 均为 10 QPS。

| 系统 | 检索 P99 (ms) | 检索 P90 (ms) | 检索平均值 (ms) | 写入 P99 (ms) | 写入 P90 (ms) | 写入平均值 (ms) |
|---|---:|---:|---:|---:|---:|---:|
...
在服务器横向对比中，以平均延迟计算，Mandol 相比最佳非 Mandol 基线实现了 **5.4 倍检索加速**（440.5 / 82.2）和 **4.8 倍记忆写入加速**（191.9 / 39.7）。

**本地部署。** 在 NVIDIA RTX 5090 Laptop 24GB 上，Mandol 在 5 QPS 检索负载下的平均检索延迟为 166.5 ms，在 10 QPS 写入负载下的平均记忆写入延迟为 37.4 ms。

| 系统 | 检索 P99 (ms) | 检索 P90 (ms) | 检索平均值 (ms) | 写入 P99 (ms) | 写入 P90 (ms) | 写入平均值 (ms) |
|---|---:|---:|---:|---:|---:|---:|
| **Mandol (Ours)** | **211.6** | **186.8** | **166.5** | **51.6** | **42.1** | **37.4** |

由于本地实验的硬件和检索 QPS 与服务器横向对比不同，该结果单独报告，不参与服务器系统排名。<sup>†</sup> EverMemOS 使用其官方实现复现。以上数据对应冻结的 [`paper-repro`](https://github.com/AgentCombo/Mandol/tree/paper-repro) 论文复现版本。
