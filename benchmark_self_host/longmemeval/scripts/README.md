# LongMemEval Self-Host Reproduction Scripts
...
```bash
RUN_ROOT=benchmark_self_host/longmemeval/test_runs/20260519_120000__gen-gpt-4o-mini-closeai__judge-gpt-4o-mini-closeai \
...
- `SAMPLE_IDS="qa_0 qa_371"`: limit to specific samples.
- `START_INDEX=0`, `END_INDEX=499`, `LIMIT=50`: range controls for build/retrieve.
- `MAX_QUESTIONS=20`: limit generate/score questions.
- `FORCE=1`, `SKIP_EXISTING=1`, `NO_RESUME=1`: pass common build controls.
- `ALLOW_PARTIAL_EXTRACTION=0`: fail on any extraction group error instead of continuing.
