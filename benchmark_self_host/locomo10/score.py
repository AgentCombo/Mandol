#!/usr/bin/env python3
"""Score LoCoMo10 self-host generation outputs with the LLM judge."""
...
from benchmark_self_host.locomo10.evaluation import (  # noqa: E402
	calculate_comprehensive_scores,
	convert_numpy_types,
)


LOGGER = logging.getLogger("locomo10_score")
...
DEFAULT_METRICS = ["exact_match", "f1", "rouge", "semantic_similarity", "llm_judge"]
...
def score_record(record: Dict[str, Any], llm_client: Optional[LLMClient], args: argparse.Namespace) -> Dict[str, Any]:
	...
	context = str(record.get("prompt") or "") if args.use_prompt_context else ""
...
def summarize_records(sample_id: str, records: List[Dict[str, Any]]) -> Dict[str, Any]:
	...
	by_category: Dict[str, Dict[str, Any]] = defaultdict(lambda: {"total": 0, "evaluated": 0, "correct": 0, "wrong": 0})
	...
	for record in records:
		category = str(record.get("category", 0))
...
def score_records(records: List[Dict[str, Any]], llm_client: Optional[LLMClient], args: argparse.Namespace) -> List[Dict[str, Any]]:
	...
	with ThreadPoolExecutor(max_workers=args.workers, thread_name_prefix="LocomoScore") as executor:
...
def build_arg_parser() -> argparse.ArgumentParser:
	parser = argparse.ArgumentParser(description="Score benchmark_self_host/locomo10 generation outputs with LLM judge.")
	parser.add_argument("--generation-dir", default=str(SCRIPT_DIR / "generate"))
	...
	parser.add_argument("--categories", nargs="+", type=int, help="Only score these categories")
	...
	parser.add_argument("--use-prompt-context", action="store_true", help="Pass saved generation prompt as judge context if present")
	parser.add_argument("--model", default="gpt-4o-mini-closeai", choices=MODEL_CHOICES_WITH_ALIASES, help="Judge model name or alias")
	...
	summary.update(
		{
			...
			"by_category": global_summary["by_category"],
		}
	)
	write_json(output_dir / "score_summary.json", summary)
