#!/usr/bin/env python3
"""Utilities for locomo benchmark episodic."""
...
logger = create_module_logger("locomo_benchmark_episodic")
...
class EpisodicRetrievalResult:
    test_case: LoCoMoEpisodicTestCase
...
class LoCoMoEpisodicBenchmark:
    ...
    def __init__(self, 
                 ...
                 output_dir: str = "locomo_episodic_benchmark_results",
        ...
        self.test_cases: List[LoCoMoEpisodicTestCase] = []
        self.test_results: List[EpisodicRetrievalResult] = []
        ...
        logger.info("LoCoMo情景记忆Benchmark测试器初始化完成")
        logger.info(f"多线程工作线程数: {self.max_workers}")
        logger.info(f"检索top-k: {self.topk}")
    ...
    def _extract_retrieval_hints(self, question: str, category: int) -> Dict[str, Any]:
        ...
        hints = {
            "question_type": self._analyze_question_type(question),
            "category": category,
            "requires_multi_hop": category in [1],
        ...
        if hints["requires_temporal"]:
            hints["target_fact_types"] = ["EVENT", "ACTIVITY", "STATE_CHANGE"]
        elif hints["requires_spatial"]:
            hints["target_fact_types"] = ["EVENT", "ACTIVITY"]
        elif "who" in question.lower():
            hints["target_fact_types"] = ["RELATIONSHIP", "OPINION", "ACTIVITY"]
        
        return hints

    def run_comprehensive_benchmark(self):
        """Run comprehensive benchmark."""
        logger.info("开始运行情景记忆benchmark测试...")
        
        test_config = {
            "name": "Triple_Fusion_BAAI_Rerank_Episodic",
            "config": {
    ...
    def _run_single_threaded_tests(self, test_config: Dict[str, Any], total_tests: int):
        ...
        for i, test_case in enumerate(tqdm(self.test_cases, desc="执行测试"), 1):
            try:
                result = self._run_single_episodic_test(test_case, test_config)
                if result:
    ...
    def _run_multi_threaded_tests(self, test_config: Dict[str, Any], total_tests: int):
        ...
        with ThreadPoolExecutor(max_workers=self.max_workers) as executor:
            future_to_test_case = {
                executor.submit(self._run_single_episodic_test_thread_safe, test_case, test_config): test_case
    ...
    def _run_single_episodic_test_thread_safe(self, test_case: LoCoMoEpisodicTestCase, method_config: Dict[str, Any]) -> Optional[EpisodicRetrievalResult]:
        """Run single episodic test thread safe."""
        try:
            return self._run_single_episodic_test(test_case, method_config)
        except Exception as e:
            logger.error(f"线程安全测试失败: {test_case.sample_id} - {e}")
            return None

    def _run_single_episodic_test(self, test_case: LoCoMoEpisodicTestCase, method_config: Dict[str, Any]) -> Optional[EpisodicRetrievalResult]:
        """Run single episodic test."""
        ...
        try:
            ...
            final_answer, confidence_score, reasoning = self._generate_answer_with_time_injection(
            ...
            retrieval_details.update({
                "answer_generation_method": "llm_synthesis_with_time_injection"
            })
            ...
            logger.error(f"单个情景记忆检索测试执行失败: {e}")
            logger.debug(traceback.format_exc())
            return None
    ...
    def _extract_time_info(self, unit) -> str:
        ...
        try:
            ...
            if time_start and time_start != "":
                return time_start
            
            time_original = unit.metadata.get("time_original", "")
            if time_original and time_original != "":
    ...
    def _build_context_with_time_injection(self, retrieved_units: List[Tuple[Any, float]]) -> str:
        """Build context with time injection."""
        context_parts = []
        ...
        for i, (unit, score) in enumerate(retrieved_units):
            try:
                ...
                context_parts.append(f"Fact {i+1}: {enhanced_content}")
                
            except Exception as e:
                logger.warning(f"处理检索单元失败: {e}")
                continue
        
        return "\n\n".join(context_parts)
    
    def _generate_answer_with_time_injection(self, question: str, retrieved_units: List[Tuple[Any, float]], hints: Dict[str, Any]) -> Tuple[str, float, str]:
        """Generate answer with time injection."""
        if not retrieved_units:
            return "No relevant information found.", 0.0, ""
        
        context = self._build_context_with_time_injection(retrieved_units)
        
        if not context:
            return "No textual content found in retrieved results.", 0.0, ""
        
        total_confidence = sum(score for _, score in retrieved_units)
        ...
        requires_temporal = hints.get("requires_temporal", False)
        
        if is_adversarial:
            prompt = f"""Based on the following context, answer the question. CRITICAL: This is an ADVERSARIAL question.

            The context contains facts with explicit time markers in [Time: ...] format. Use these timestamps to verify information.
            ...
            INSTRUCTIONS:
            1. Carefully verify if the information is EXPLICITLY present in the context
            2. Pay attention to the [Time: ...] markers for temporal accuracy
            3. If information is not found or question is misleading, state that clearly
        ...
        elif requires_temporal or question_type == "when":
            prompt = f"""Based on the following context, answer the question about TIME.

            IMPORTANT: The context contains facts with explicit time markers in [Time: YYYY-MM-DD] or [Time: relative_text] format.
            - Use these [Time: ...] markers to provide accurate temporal information
            - For "when" questions, extract the specific date/time from the relevant fact's [Time: ...] marker
            - If multiple time references exist, identify the one most relevant to the question

            Question: {question}

            Context with Time Markers:
            {context}

            Provide a direct answer with the specific date or time period. If the information is not available, state "No information available".
            Answer:"""
        
        elif question_type == "where":
            prompt = f"""Based on the following context, answer the question about LOCATION.

            The context contains facts with time markers [Time: ...]. Focus on location-related information.
            ...
            Provide a direct answer. If the information is not available, state "No information available".
            Answer:"""
        
        elif question_type == "who":
            prompt = f"""Based on the following context, answer the question about PEOPLE.

            The context contains facts with time markers [Time: ...]. Focus on people and relationships.
            ...
            IMPORTANT: The context contains facts with explicit [Time: ...] markers indicating when events occurred.
            Use these time markers to provide temporally accurate answers.
            ...
            Provide a direct and concise answer. If the information is not available, state "No information available".
            Answer:"""
    ...
    def generate_benchmark_report(self):
        ...
        benchmark_report = {
            "benchmark_info": {
                "test_name": "LoCoMo Episodic Memory Benchmark",
                "test_type": "episodic_memory_retrieval_with_time_injection",
                ...
                "method_used": "Triple_Fusion_Rerank_Episodic_TimeInjection",
            ...
            "detailed_results": [
                {
                    ...
                    "evaluation_scores": convert_numpy_types(r.evaluation_scores),
        ...
        report_file = self.output_dir / f"locomo_episodic_benchmark_report_{timestamp}.json"
    ...
    def _calculate_overall_statistics(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_retrieval_time": float(np.mean(all_retrieval_times)),
            "avg_f1_score": float(np.mean(all_f1_scores)),
            "avg_semantic_similarity": float(np.mean(all_semantic_scores)),
            "avg_llm_accuracy": float(np.mean(all_llm_scores)),
            "avg_exact_match": float(np.mean(all_exact_match)),
            "std_f1_score": float(np.std(all_f1_scores)),
            "std_semantic_similarity": float(np.std(all_semantic_scores))
        }
    
    def _calculate_sample_performance(self, sample_results: List[EpisodicRetrievalResult]) -> Dict[str, Any]:
    ...
    def _calculate_question_type_performance(self, type_results: List[EpisodicRetrievalResult]) -> Dict[str, Any]:
        ...
        return {
            ...
            "avg_f1_score": float(np.mean([r.evaluation_scores.get("token_f1", 0.0) for r in type_results])),
            "avg_semantic_similarity": float(np.mean([r.evaluation_scores.get("semantic_similarity", 0.0) for r in type_results])),
            "avg_llm_accuracy": float(np.mean([r.evaluation_scores.get("llm_accuracy", 0.0) for r in type_results])),
            "avg_retrieval_time": float(np.mean([r.retrieval_time for r in type_results])),
            "success_rate": len([r for r in type_results if r.evaluation_scores.get("token_f1", 0.0) > 0.5]) / len(type_results)
        }
    
    def _calculate_category_performance(self, category_results: List[EpisodicRetrievalResult]) -> Dict[str, Any]:
    ...
    def _generate_readable_report(self, benchmark_report: Dict[str, Any], timestamp: str = None):
        ...
        lines.append("=" * 80)
        lines.append("LoCoMo情景记忆（Episodic Memory）Benchmark测试报告")
        lines.append(" 核心特性: 时间信息注入到检索上下文中")
        lines.append("=" * 80)
        ...
        lines.append(f"   - 检索top-k: {info['topk']}")
        lines.append(f"   - 重排序器: {info['reranker_type']}")
        ...
        for category, stats in cat_perf.items():
            category_name = category_names.get(int(category), f"类别{category}")
            lines.append(f"\n   {category_name}:")
            lines.append(f"     - 测试数量: {stats['test_count']}")
            lines.append(f"     - 平均F1: {stats['avg_f1_score']:.3f}")
            lines.append(f"     - 平均语义相似度: {stats['avg_semantic_similarity']:.3f}")
        ...
        if timestamp:
            readable_report_file = self.output_dir / f"locomo_episodic_benchmark_readable_report_{timestamp}.txt"
        else:
            readable_report_file = self.output_dir / "locomo_episodic_benchmark_readable_report.txt"
...
def main():
    ...
    parser = argparse.ArgumentParser(description="LoCoMo情景记忆（Episodic Memory）Benchmark测试")
    ...
    parser.add_argument("--output-dir", 
                       default=str(paths.LOCOMO_TASK_EVAL_RESULTS_DIR / "locomo_episodic_benchmark_results"),
                       help="输出目录")
    ...
    print("=" * 80)
    print(" LoCoMo情景记忆（Episodic Memory）Benchmark测试系统")
    print(" 核心特性: 时间信息注入到检索上下文中")
    print("=" * 80)
    ...
    try:
        ...
        print("\n 初始化Benchmark测试器...")
        benchmark = LoCoMoEpisodicBenchmark(
