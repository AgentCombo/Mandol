...
const en: Translations = {
  ...
  heroPipInstall: 'uv sync --extra dev --extra docs --group spacy-model',
  ...
  whatIsDesc2:
    'Retrieval is handled through MultiRetriever for BM25, SPLADE, cosine search, graph expansion, score fusion, and reranker orchestration. Complete snapshots use SemanticGraph.save_graph() and SemanticGraph.load_graph().',
  ...
  innovation2_points: [
    ...
    'Sandboxed graph snapshots with DuckDB L2 support',
  ],
...
const zhHans: Translations = {
  ...
  whatIsDesc2:
    '检索由 MultiRetriever 负责，覆盖 BM25、SPLADE、余弦搜索、图扩展、分数融合和 reranker 编排。完整快照使用 SemanticGraph.save_graph() 与 SemanticGraph.load_graph()。',
