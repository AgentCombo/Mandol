...
def calculate_llm_judgment(llm_client: LLMClient, 
    ...
    for i in range(num_runs):
        try:
            ...
            logging.debug(f"LLM {i+1}/{num_runs}: {judgment}")
        except Exception as e:
...
def test_llm_grader(llm_client: LLMClient):
    ...
    for i, case in enumerate(test_cases, 1):
        ...
        print(f": {result}")
        print(f": {'' if result == case['expected'] else ''}")
...
def llm_judge_adversarial(llm_client: LLMClient,
    ...
    try:
        ...
        try:
            ...
            logging.warning(f"JSON parse failedusing text match: {response}")
            response_upper = response.upper()
...
def example_llm_grader_usage():
    ...
    print(f": {json.dumps(comprehensive_result, indent=2)}")
