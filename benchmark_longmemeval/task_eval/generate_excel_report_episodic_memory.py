...
# -*- coding: utf-8 -*-
"""
Generate Excel Analysis Report for Episodic Memory Benchmark Results

This script reads all qa_*.json files from the individual_reports directory
of episodic_memory benchmark, aggregates the results, and creates a comprehensive
Excel report.

Designed specifically for episodic_memory benchmark JSON structure:
- retrieval_details.total_retrieved
- retrieval_details.facts_by_category
- retrieval_details.stable_facts_count, temporal_facts_count
- retrieval_details.avg_confidence, rerank_time
- Standard LongMemEval question types
...
def compute_summary(detailed_results: List[Dict]) -> Dict:
    ...
    if detailed_results:
        ...
        retrieval_config = {
            ...
            'top_k': retrieval_details.get('total_retrieved', 10)
        }
    ...
    for result in detailed_results:
        ...
        # Retrieval time and rerank time (episodic_memory specific)
        ...
        rerank_time = ret_details.get('rerank_time', 0)
        rerank_times.append(rerank_time)
        by_question_type[question_type]['rerank_times'].append(rerank_time)
    ...
    for qtype, data in by_question_type.items():
        by_type_summary[qtype] = {
            ...
            'avg_rerank_time': statistics.mean(data['rerank_times']) if data['rerank_times'] else 0
        }
    
    # Build summary
    summary = {
        'baseline_version': 'episodic_memory_no_expand',
        ...
        'avg_rerank_time': statistics.mean(rerank_times) if rerank_times else 0,
...
def create_excel_report(detailed_results: List[Dict], summary: Dict, output_path: str, zep_model: str = 'gpt-4o-mini'):
    ...
    # Episodic Memory Specific: Facts Statistics
    create_facts_statistics_sheet(results, writer)
...
def create_summary_sheet_horizontal(results: Dict, writer, zep_model: str = 'gpt-4o-mini'):
    ...
    data = {
        ...
        '平均重排时间(秒)': [f"{summary.get('avg_rerank_time', 0):.2f}"],
...
def create_question_type_comparison_sheet(results: Dict, writer, zep_model: str = 'gpt-4o-mini'):
    ...
    data = {
        '指标': ['样本数', 'LLM准确率', f'ZEP准确率({zep_model})', 'F1分数', '平均检索时间(秒)', '平均重排时间(秒)']
    }
    ...
    for qtype in question_types:
        ...
        data[qtype] = [
            ...
            f"{stats.get('avg_rerank_time', 0):.2f}"
        ]
...
def create_question_type_comparison_transposed(results: Dict, writer, zep_model: str = 'gpt-4o-mini'):
    ...
    for qtype in question_types:
        ...
        row = {
            ...
            '平均重排时间(秒)': f"{stats.get('avg_rerank_time', 0):.2f}"
        }
        data.append(row)
...
def create_individual_question_type_sheets(results: Dict, writer):
    ...
    # Standard LongMemEval question type name mapping
    sheet_name_map = {
    ...
    for qtype in sorted(by_type.keys()):
        ...
        for result in type_results:
            ...
            data.append({
                ...
                '重排时间(s)': f"{retrieval.get('rerank_time', 0):.2f}"
            })
...
def create_retrieval_sheet(results: Dict, writer):
    ...
    for result in detailed_results:
        ...
        rerank_time = retrieval.get('rerank_time', 0)
        # Episodic memory uses total_retrieved
        ret_count = retrieval.get('total_retrieved', 0)
        ...
        by_type[qtype]['rerank_times'].append(rerank_time)
    ...
    data['平均重排时间(秒)'].append(f"{statistics.mean(rerank_times):.2f}" if rerank_times else 0)
    ...
    for qtype in sorted(by_type.keys()):
        ...
        data['平均重排时间(秒)'].append(f"{statistics.mean(metrics['rerank_times']):.2f}")
    ...
    for result in detailed_results:
        ...
        if llm_acc < 0.5:  # Consider as error if accuracy < 0.5
            ...
            errors.append({
                ...
                '检索数量': retrieval.get('total_retrieved', 0),
                '稳定事实数': retrieval.get('stable_facts_count', 0),
                '时序事实数': retrieval.get('temporal_facts_count', 0),
...
def create_facts_statistics_sheet(results: Dict, writer):
    """Create facts statistics sheet (specific to episodic_memory benchmark)."""
    import pandas as pd
    
    detailed_results = results.get('detailed_results', [])
    
    # Aggregate facts statistics
    stable_facts_counts = []
    temporal_facts_counts = []
    avg_confidences = []
    
    facts_by_category_all = defaultdict(int)
    
    for result in detailed_results:
        ret_details = result.get('retrieval_details', {})
        
        stable_count = ret_details.get('stable_facts_count', 0)
        temporal_count = ret_details.get('temporal_facts_count', 0)
        avg_conf = ret_details.get('avg_confidence', 0)
        
        stable_facts_counts.append(stable_count)
        temporal_facts_counts.append(temporal_count)
        if avg_conf:
            avg_confidences.append(avg_conf)
        
        # Aggregate facts by category
        facts_by_cat = ret_details.get('facts_by_category', {})
        for cat, count in facts_by_cat.items():
            facts_by_category_all[cat] += count
    
    # Create facts overview data
    data = {
        '指标': [
            '平均稳定事实数',
            '平均时序事实数',
        ...
        '数值': [
            f"{statistics.mean(stable_facts_counts):.2f}" if stable_facts_counts else 0,
            f"{statistics.mean(temporal_facts_counts):.2f}" if temporal_facts_counts else 0,
            f"{statistics.mean(avg_confidences):.4f}" if avg_confidences else 'N/A',
            sum(stable_facts_counts),
            sum(temporal_facts_counts),
            len(detailed_results)
        ]
    }
    
    df1 = pd.DataFrame(data)
    df1.to_excel(writer, sheet_name='事实统计-概览', index=False)
    
    # Create facts by category distribution
    if facts_by_category_all:
        category_data = [
            {'事实类别': cat, '总数': count, '平均每QA': f"{count / len(detailed_results):.2f}"}
            for cat, count in sorted(facts_by_category_all.items(), key=lambda x: x[1], reverse=True)
        ]
        df2 = pd.DataFrame(category_data)
    else:
        df2 = pd.DataFrame({'信息': ['没有事实类别统计数据']})
    
    df2.to_excel(writer, sheet_name='事实类别分布', index=False)
...
def find_individual_reports_dir(start_path: str = '.') -> Optional[str]:
    """
    Find the most recent individual_reports directory for episodic_memory.
    """
    start = Path(start_path)
    
    # Search patterns
    search_patterns = [
        start / 'results' / 'episodic_memory_no_expand',
        start / 'benchmark_longmemeval' / 'task_eval' / 'results' / 'episodic_memory_no_expand',
        Path('.') / 'benchmark_longmemeval' / 'task_eval' / 'results' / 'episodic_memory_no_expand',
    ]
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='生成Episodic Memory基准测试的Excel分析报告（横版布局，ZEP风格）'
    )
    parser.add_argument(
        '--input_dir',
        type=str,
    ...
    if args.input_dir:
        ...
        print(" 自动查找最新的episodic_memory individual_reports目录...")
    ...
    if args.output:
        ...
        output_path = parent_dir / f"episodic_memory_analysis_{timestamp}.xlsx"
    ...
    if success:
        ...
        print("  错误分析 - 低准确率案例")
        print("  事实统计-概览 - Episodic Memory特有")
        print("  事实类别分布 - Episodic Memory特有")
    else:
        print("\n 生成报告失败")
