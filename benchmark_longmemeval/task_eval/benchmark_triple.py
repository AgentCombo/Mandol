#!/usr/bin/env python3
"""Utilities for benchmark triple."""
...
def parse_sample_ids(value: Optional[str]) -> Optional[List[int]]:
    """Parse comma-separated QA ids and inclusive ranges, e.g. '1,3,10-12'."""
    if value is None:
        return None
    ...
    for part in value.split(","):
        ...
        if "-" in part:
            ...
            if start > end:
                raise ValueError(f"无效 sample id 范围: {part}")
...
class LongMemEvalTripleFusionBenchmark:
    ...
    def __init__(self,
        ...
        self.end_qa = end_qa
        self.sample_ids = sorted(set(sample_ids)) if sample_ids else None
    ...
    def _print_config(self):
        ...
        if self.sample_ids:
            preview = ",".join(str(i) for i in self.sample_ids[:20])
            suffix = "..." if len(self.sample_ids) > 20 else ""
            logger.info(f" 指定 QA IDs: {preview}{suffix} (共 {len(self.sample_ids)} 个)")
    ...
    def _load_test_cases(self) -> List[TripleTestCase]:
        ...
        if self.sample_ids is not None:
            candidate_indices = [
                idx for idx in self.sample_ids
                if 0 <= idx < len(dataset) and start_idx <= idx <= end_idx
            ]
        else:
        ...
        if self.sample_ids is not None:
            logger.info(f" 加载 {len(test_cases)} 个指定测试用例")
        else:
...
def main():
    ...
    parser.add_argument("--output-dir",
                       default=str(paths.LONGMEMEVAL_TASK_EVAL_RESULTS_DIR / "triple_fusion"),
                       help="结果输出目录")
    ...
    parser.add_argument("--sample-ids", default=None,
                       help="只运行指定 QA 索引，支持逗号和范围，例如 8,34,70-75")
    ...
    try:
        ...
        logger.error(f" sample-ids 参数无效: {e}")
        return 1
    if sample_ids:
        invalid_ids = [idx for idx in sample_ids if idx < 0 or idx > 499]
        if invalid_ids:
            logger.error(f" sample-ids 必须在 0-499 范围内，非法值: {invalid_ids[:10]}")
            return 1
