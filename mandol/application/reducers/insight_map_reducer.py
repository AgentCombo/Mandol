...
class InsightMapReducer:
    ...
    def _map_phase(
        ...
        try:
            ...
            return self._parse_insight_response(response.content, session_id, source_summary_uids)
        except Exception as e:
            logger.error(f"Insight map phase failed: {e}")
            return self._create_fallback_insight(session_id, source_summary_uids)
    ...
    def _parse_insight_response(
        ...
        try:
            ...
            logger.warning(f"Failed to parse insight response: {response[:200]}")
            return self._create_fallback_insight(session_id, source_summary_uids)
    ...
    def reduce_insights(
        ...
        prompt = f"""Given existing session insights and new session insights, merge them intelligently into a global insight.
...
Merge rules:
1. Keep important insights from existing sessions
2. Merge highly similar/overlapping insights
3. Add genuinely new insights from new sessions
4. Set key_source_session_ids to all contributing session IDs

Output JSON only:"""

        messages: List[ChatMessage] = [
            {"role": "system", "content": INSIGHT_REDUCE_SYSTEM_PROMPT},
            {"role": "user", "content": prompt},
        ]
        ...
        try:
            ...
            return self._parse_reduce_response(response.content, all_session_ids)
        except Exception as e:
    ...
    def _parse_reduce_response(
        ...
        try:
            ...
            logger.warning(f"Failed to parse reduce response: {response[:200]}")
            return self._merge_insights_fallback([], session_ids)
