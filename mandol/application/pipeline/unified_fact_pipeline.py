...
class ExtractedEvent:
    ...
    Attributes:
        ...
        location: Optional location dict with linked_id.
...
class UnifiedFactPipeline:
    ...
    def process_session(
        ...
        extracted_events = self._extract_events(dialogue_text, existing_events, current_entities)

        entity_relations = self._extract_entity_relations(dialogue_text, current_entities)
    ...
    def _retrieve_existing_entities(self, dialogue_text: str) -> List[Dict[str, Any]]:
        ...
        try:
            query_emb = self._embedder.embed(dialogue_text)
        except Exception:
            return results
        ...
        for hit, _ in hits:
            ...
            results.append({
                "id": eid,
                "name": entity_name,
                "type": hit.unit.raw_data.get("entity_type", ""),
                "aliases": hit.unit.raw_data.get("aliases", []),
            })
    ...
    def _retrieve_existing_events(self, dialogue_text: str) -> List[Dict[str, Any]]:
        ...
        return [
            {
                "id": str(hit.unit.uid),
                "name": _extract_event_name_from_text_content(hit.unit.raw_data.get("text_content", "")),
                "signature": hit.unit.metadata.get("signature", ""),
            }
            for hit, _ in hits
        ]
    ...
    def _call_llm(self, prompt: str) -> LLMChatResponse:
        ...
        Returns:
            LLMChatResponse with parsed content.
        """
