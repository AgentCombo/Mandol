...
class ProviderFactoryResult:
    ...
    All fields are ready-to-use instances (or None for optional components).
    Consumers should iterate over non-None index fields to determine
    which retrieval strategies are active.

    Attributes:
        embedder: Text/image embedding provider, or None if skipped.
        reranker: Cross-encoder reranker, or None if skipped.
        llm: LLM provider, or None if skipped.
        unit_store: Primary unit and space storage.
        graph_store: Graph relationship store, or None.
        vector_index: Optional additional vector index (e.g., adaptive).
        bm25_index: Optional BM25 lexical index.
        sparse_index: Optional sparse vector index.
    """
...
def _build_embedder(cfg: Mapping[str, Any]) -> Optional[EmbeddingProvider]:
    ...
    if mode == "remote":
        ...
        cfg_obj = OpenAICompatibleEmbeddingConfig(
            ...
            timeout_s=int(
                timeout_s
                or int(os.getenv("MANDOL_EMBEDDER_TIMEOUT_S", "30"))
            ),
        )
...
def _build_reranker(cfg: Mapping[str, Any]) -> Optional[Reranker]:
    ...
    if mode == "remote":
        ...
        cfg_obj = OpenAICompatibleRerankConfig(
            ...
            timeout_s=int(
                timeout_s
                or int(os.getenv("MANDOL_RERANKER_TIMEOUT_S", "30"))
            ),
